...
const createService = (
  ...
  const patientIdentities = {
    ...
    updateById: vi.fn(async (id: string, update: Record<string, unknown>) => {
      const identity = [...identitiesByPhone.values()].find(
        (item) => String(item._id) === id,
      );
  ...
  const patientProfiles = {
    findById: vi.fn(
      async (id: string) =>
        [...compatProfilesByPhone.values()].find(
          (item) => String(item._id) === id,
        ) ?? null,
    ),
    findByOrganizationAndNormalizedPhone: vi.fn(
      async (_organizationId: string, normalizedPhone: string) =>
        compatProfilesByPhone.get(normalizedPhone) ?? null,
    ),
    updateById: vi.fn(async (id: string, update: Record<string, unknown>) => {
      const profile = [...compatProfilesByPhone.values()].find(
        (item) => String(item._id) === id,
      );
  ...
  const organizationPatientProfiles = {
    findByOrganizationAndIdentity: vi.fn(
      async (_organizationId: string, patientIdentityId: string) =>
        orgProfilesByIdentity.get(patientIdentityId) ?? null,
    ),
    findByOrganizationAndNormalizedPhone: vi.fn(
      async (_organizationId: string, normalizedPhone: string) =>
        orgProfilesByPhone.get(normalizedPhone) ?? null,
    ),
    updateById: vi.fn(async (id: string, update: Record<string, unknown>) => {
      const profile = [...orgProfilesByIdentity.values()].find(
        (item) => String(item._id) === id,
      );
      if (!profile) return null;
      Object.assign(profile, update);
  ...
  const appointmentsService = {
    createExpressAppointment: vi.fn(
      async (_organizationId: string, input: Record<string, unknown>) => ({
  ...
  const service = new PatientService(
    ...
    {
      upsertActive: vi.fn(async (input: Record<string, unknown>) => ({
        _id: new mongoose.Types.ObjectId(),
        ...input,
      })),
    } as never,
  ...
  return {
    ...
    patientIdentities,
    patientProfiles,
    organizationPatientProfiles,
    appointmentsService,
    expressSessions,
    healthInsurances,
    identitiesByPhone,
    compatProfilesByPhone,
    orgProfilesByIdentity,
  };
};
...
describe("PatientService createExpressAppointment express identity/profile/appointment chain", () => {
  it("creates separate PatientIdentities and profiles for two different normalized phones without overwriting the first patient", async () => {
    ...
    await service.createExpressAppointment(
      "centro-demo",
      baseInput("+54 11 1111-1111"),
    );
    await service.createExpressAppointment(
      "centro-demo",
      baseInput("+54 11 2222-2222", "2026-07-01T15:00:00.000Z"),
    );

    expect(patientIdentities.findByNormalizedPhone).toHaveBeenNthCalledWith(
      1,
      "541111111111",
    );
    expect(patientIdentities.findByNormalizedPhone).toHaveBeenNthCalledWith(
    ...
    expect(identitiesByPhone.get("541111111111")).toEqual(
      expect.objectContaining({
        normalizedPhone: "541111111111",
        firstName: "Ana",
      }),
    );
    expect(identitiesByPhone.get("541122222222")).toEqual(
      expect.objectContaining({
        normalizedPhone: "541122222222",
        firstName: "Ana",
      }),
    );
    expect(patientProfiles.create).toHaveBeenNthCalledWith(
      1,
      expect.objectContaining({
        organizationId,
        userId: null,
    ...
    expect(appointmentsService.createExpressAppointment).toHaveBeenCalledTimes(
      2,
    );
  });
  ...
  it("reuses PatientIdentity and PatientProfile for the same phone in the same organization and still creates another appointment", async () => {
    ...
    const firstAppointment = await service.createExpressAppointment(
      "centro-demo",
      baseInput("+54 11 3333-3333"),
    );
    const secondAppointment = await service.createExpressAppointment(
      "centro-demo",
      baseInput("+54 11 3333-3333", "2026-07-01T16:00:00.000Z"),
    );
    ...
    expect(secondAppointment.patientProfileId).toBe(
      firstAppointment.patientProfileId,
    );
  });
  ...
  it("recovers from a duplicate PatientIdentity race by re-reading the identity by normalizedPhone", async () => {
    const racedIdentity = {
      _id: new mongoose.Types.ObjectId(),
      normalizedPhone: "541144444444",
      firstName: "Ana",
      lastName: "Paz",
    };
    const duplicateKeyError = Object.assign(
      new Error("E11000 duplicate key error normalizedPhone_1_unique"),
      {
        code: 11000,
    ...
    const findByNormalizedPhone = vi
      .fn()
    ...
    await expect(
      service.createExpressAppointment(
        "centro-demo",
        baseInput("+54 11 4444-4444"),
      ),
    ).resolves.toEqual(
      expect.objectContaining({ patientProfileId: expect.any(String) }),
    );
  ...
  it("returns only masked data when looking up an existing phone", async () => {
    ...
    const result = await service.lookupExpressPatient("centro-demo", {
  ...
  it("prefills safe saved patient data only after explicit acceptance", async () => {
    ...
    const { service } = createService({
      healthInsurances: {
        findByIdInOrganization: vi.fn(async () => ({
          _id: healthInsuranceId,
          name: "OSDE",
          status: "active",
    ...
    const prefill = await service.prefillExpressPatient("centro-demo", {
      phone: "+54 11 3333-6516",
  ...
  it("requires explicit saved data acceptance before returning prefill data", async () => {
    ...
    await expect(
      service.prefillExpressPatient("centro-demo", {
        phone: "+54 11 3333-6516",
        acceptSavedData: false as true,
      }),
  ...
  it("returns global identity data with private coverage when no organization profile exists", async () => {
    ...
    (
      service as unknown as {
        organizations: { findBySlug: ReturnType<typeof vi.fn> };
      }
    ).organizations.findBySlug.mockResolvedValueOnce({
      _id: otherOrganizationId,
      status: "active",
      name: "Otro Centro",
      displayName: "Otro Centro",
      slug: "otro-centro",
      type: "clinic",
    });

    await expect(
      service.prefillExpressPatient("otro-centro", {
        phone: "+54 11 3333-6516",
        acceptSavedData: true,
      }),
    ).resolves.toEqual({
  ...
  it("creates an appointment from a WhatsApp lookup token without requiring manual patient data", async () => {
    ...
    const lookup = await service.lookupExpressPatient("centro-demo", {
      phone: "+54 11 3333-6516",
    });
    ...
    expect(
      appointmentsService.createExpressAppointment,
  ...
  it("prioritizes payload coverage over saved coverage when booking with saved patient data", async () => {
    const savedHealthInsuranceId = new mongoose.Types.ObjectId().toString();
    const { service, appointmentsService } = createService({
      healthInsurances: {
        findByIdInOrganization: vi.fn(async () => ({
          _id: savedHealthInsuranceId,
          name: "OSDE",
          status: "active",
          requiresMemberNumber: false,
          requiresPlan: false,
    ...
    await service.createExpressAppointment("centro-demo", {
      ...
      coverage: {
        type: "health_insurance",
        healthInsuranceId: savedHealthInsuranceId,
        insuranceMemberNumber: "123456",
        insurancePlan: "210",
      },
    });
    ...
    await service.createExpressAppointment("centro-demo", {
      professionalId: new mongoose.Types.ObjectId().toString(),
      specialtyId: new mongoose.Types.ObjectId().toString(),
      startAt: "2026-07-04T16:00:00.000Z",
      useSavedPatientData: true,
      patientLookupToken: lookup.found ? lookup.lookupToken : undefined,
  ...
  it("returns a clear 400 when saved-data payload selects inactive health insurance", async () => {
    const inactiveHealthInsuranceId = new mongoose.Types.ObjectId().toString();
    const { service } = createService({
      healthInsurances: {
        findByIdInOrganization: vi.fn(async () => ({
          _id: inactiveHealthInsuranceId,
          name: "OSDE",
          status: "inactive",
          requiresMemberNumber: false,
          requiresPlan: false,
    ...
    await expect(
      service.createExpressAppointment("centro-demo", {
        professionalId: new mongoose.Types.ObjectId().toString(),
        specialtyId: new mongoose.Types.ObjectId().toString(),
        ...
        coverage: {
          type: "health_insurance",
          healthInsuranceId: inactiveHealthInsuranceId,
        },
      }),
    ).rejects.toMatchObject({
      statusCode: 400,
      message: "La cobertura seleccionada no está disponible",
    });
  });
  ...
  it("creates an appointment with the current express patient without requiring personal data again", async () => {
    ...
    await service.createExpressAppointment(
      "centro-demo",
      baseInput("+54 11 4444-6516"),
    );
    const identity = identitiesByPhone.get("541144446516");
    expect(identity).toBeTruthy();
    vi.mocked(expressSessions.findByTokenHash).mockResolvedValueOnce({
      _id: new mongoose.Types.ObjectId(),
      patientIdentityId: identity!._id,
      expiresAt: new Date(Date.now() + 60_000),
    } as never);
    patientIdentities.create.mockClear();

    const appointment = await service.createExpressAppointment(
      "centro-demo",
      {
        professionalId: new mongoose.Types.ObjectId().toString(),
        specialtyId: new mongoose.Types.ObjectId().toString(),
