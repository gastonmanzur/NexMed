...
export interface OrganizationWhatsAppSettingsDto {
  ...
  templates: { confirmation: string; reminder: string; cancellation: string; rescheduled: string; notice: string; appointmentConfirmation: string; appointmentReminder: string; appointmentCancellation: string; appointmentRescheduled: string };
...
export class OrganizationWhatsAppSettingsService {
  ...
  async upsert(organizationId: string, input: Partial<OrganizationWhatsAppSettingsDto>): Promise<OrganizationWhatsAppSettingsDto> {
    const update: any = { organizationId, enabled: input.enabled ?? false, senderDisplayName: env.META_WHATSAPP_SENDER_DISPLAY_NAME };
