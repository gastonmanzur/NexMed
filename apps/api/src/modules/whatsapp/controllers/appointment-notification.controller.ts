...
const toDto = (row: Awaited<ReturnType<AppointmentNotificationRepository['listByAppointment']>>[number]) => ({
  ...
  recipientPhone: mask(row.recipientPhone), normalizedRecipientPhone: mask(row.normalizedRecipientPhone), senderDisplayName: row.senderDisplayName ?? 'NexMed', provider: row.provider ?? 'meta_cloud_api', templateName: row.templateName ?? null, templateLanguage: row.templateLanguage ?? 'es_AR', templateParams: row.templateParams ?? [], providerMessageId: row.providerMessageId ?? null, errorMessage: row.errorMessage ?? row.error ?? null, attempts: row.attempts, lastAttemptAt: row.lastAttemptAt?.toISOString() ?? null, createdAt: row.createdAt.toISOString(), updatedAt: row.updatedAt.toISOString()
});
