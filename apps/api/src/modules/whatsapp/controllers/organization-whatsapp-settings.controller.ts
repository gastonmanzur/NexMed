...
export const organizationWhatsAppSettingsController = {
  ...
  test: async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    ...
    try {
      ...
      res.status(400).json({ success: false, error: { code, message }, data: { ok: false, notificationId: null, status: 'failed', errorCode: code, errorMessage: message, templateName: null, templateLanguage: null } });
    }
  }
};
