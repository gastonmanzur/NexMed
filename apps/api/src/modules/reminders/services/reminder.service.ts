...
export class ReminderService {
  ...
  async runDueReminders(now = new Date()): Promise<{ generated: number; scanned: number }> {
    ...
    for (const dispatch of due) {
      ...
      if (!appointment || appointment.status !== 'booked') { await this.dispatches.cancelPendingByAppointment(dispatch.appointmentId.toString(), 'appointment_not_booked_anymore'); continue; }
      try {
