...
const envSchema = z
  .object({
    ...
    META_WHATSAPP_ENABLED: z.union([z.boolean(), z.enum(['true', 'false']), z.string().length(0)]).optional(),
    META_WHATSAPP_API_VERSION: z.string().trim().min(1).default('v22.0'),
  ...
  .transform((data) => {
    ...
    return {
      ...
      WHATSAPP_META_API_VERSION: data.META_WHATSAPP_API_VERSION ?? data.WHATSAPP_META_API_VERSION,
      WHATSAPP_ACCESS_TOKEN: data.META_WHATSAPP_ACCESS_TOKEN ?? data.WHATSAPP_ACCESS_TOKEN,
