...
export const DemoTour = (): ReactElement => {
  ...
  }, [open, step.id, stepIndex]);

  const progress = ((stepIndex + 1) / demoTourSteps.length) * 100;

  if (!open) return <button className="demo-resume" onClick={() => setOpen(true)}>Reanudar tour</button>;

  return <div className="demo-tour" role="dialog" aria-modal="true">
