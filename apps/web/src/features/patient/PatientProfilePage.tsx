  ...
  return <main className="nx-page" style={{ maxWidth: 900 }}>
    <Card title="Perfil del paciente" subtitle="Ficha personal para consultorios y clínicas.">{error && <p style={{ color: 'var(--danger)' }}>{error}</p>}{message && <p style={{ color: '#166534' }}>{message}</p>}
      <form className="nx-form-grid" onSubmit={(e) => e.preventDefault()}>
        ...
        <h3>Preferencias de comunicación</h3><p>Al activarlo, los centros vinculados podrán enviarte mensajes operativos relacionados con tus turnos.</p><label className="nx-field">Preferencia contacto<select {...bind('contactPreference')}><option value="">Seleccionar</option><option value="whatsapp">WhatsApp</option><option value="email">Email</option><option value="telefono">Teléfono</option></select></label>
        ...
        <label><input type="checkbox" checked={!!profile.acceptsWhatsAppCommunications} onChange={(e) => setProfile((p) => ({ ...p, acceptsWhatsAppCommunications: e.target.checked }))} /> Recibir confirmaciones y recordatorios de turnos por WhatsApp</label>{profile.acceptsWhatsAppCommunications ? <p style={{ color: '#166534' }}>Las notificaciones de turnos por WhatsApp están activadas.</p> : <p>No recibirás confirmaciones ni recordatorios por WhatsApp.</p>}
        <button type="button" className="nx-btn" onClick={async () => { if (!accessToken) return; try { await patientApi.patchMe(accessToken, profile); setMessage('Preferencias guardadas correctamente.'); setError(''); } catch (e) { setError((e as Error).message); } }}>Guardar perfil</button>
      </form>
    </Card>
