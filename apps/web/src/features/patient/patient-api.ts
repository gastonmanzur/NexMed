...
export const patientApi = {
  ...
  createFamilyMember: async (accessToken: string, input: { firstName: string; lastName: string; relationship: string; dateOfBirth: string; documentId: string; phone?: string; notes?: string; isActive?: boolean }): Promise<PatientFamilyMemberDto> => (await request<{ success: true; data: PatientFamilyMemberDto }>('/patient/family-members', { method: 'POST', headers: { Authorization: `Bearer ${accessToken}` }, body: JSON.stringify(input) })).data,
  patchFamilyMember: async (accessToken: string, familyMemberId: string, input: { firstName?: string; lastName?: string; relationship?: string; dateOfBirth?: string; documentId?: string; phone?: string; notes?: string; isActive?: boolean }): Promise<PatientFamilyMemberDto> => (await request<{ success: true; data: PatientFamilyMemberDto }>(`/patient/family-members/${familyMemberId}`, { method: 'PATCH', headers: { Authorization: `Bearer ${accessToken}` }, body: JSON.stringify(input) })).data,
