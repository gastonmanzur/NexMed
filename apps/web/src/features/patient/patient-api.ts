...
interface ApiErrorPayload { success?: boolean; message?: string; error?: { message?: string } }
...
const request = async <T>(path: string, init: RequestInit): Promise<T> => {
  ...
  if (!response.ok) throw new PatientApiError(payload?.error?.message ?? payload?.message ?? `Request failed with status ${response.status}`, response.status);
  return payload ?? ({} as T);
};
export const patientApi = {
  ...
  createExpressAppointment: async (tokenOrSlug: string, input: { professionalId: string; specialtyId: string; startAt: string; endAt?: string; appointmentType?: 'single' | 'double'; patient: { firstName: string; lastName: string; phone: string; email?: string; documentNumber?: string; birthDate?: string }; coverage: { type: 'private' | 'health_insurance'; healthInsuranceId?: string | null; insuranceMemberNumber?: string | null; insurancePlan?: string | null }; reason?: string }): Promise<AppointmentDto> => (await request<{ success: true; data: AppointmentDto }>(`/join/${encodeURIComponent(tokenOrSlug)}/appointments/express`, { method: 'POST', body: JSON.stringify(input) })).data,
  ...
  patchMe: async (accessToken: string, input: Partial<PatientProfileDto>): Promise<PatientProfileDto> => (await request<{ success: true; data: PatientProfileDto }>('/patient/me', { method: 'PATCH', headers: { Authorization: `Bearer ${accessToken}` }, body: JSON.stringify(input) })).data,
  ...
  getOrganizationCatalog: async (accessToken: string, organizationId: string): Promise<PatientCatalog> => (await request<{ success: true; data: PatientCatalog }>(`/patient/organizations/${organizationId}/catalog`, { method: 'GET', headers: { Authorization: `Bearer ${accessToken}` } })).data,
  getAvailability: async (accessToken: string, organizationId: string, params: { professionalId: string; specialtyId: string; startDate: string; endDate: string }): Promise<CalculatedAvailabilityDto> => (await request<{ success: true; data: CalculatedAvailabilityDto }>(`/patient/organizations/${organizationId}/availability?${new URLSearchParams(params).toString()}`, { method: 'GET', headers: { Authorization: `Bearer ${accessToken}` } })).data,
  createAppointment: async (accessToken: string, organizationId: string, input: { professionalId: string; specialtyId: string; startAt: string; endAt?: string; durationMultiplier?: AppointmentDurationMultiplier; notes?: string; beneficiaryType?: 'self' | 'family_member'; familyMemberId?: string; patientProfileId?: string }): Promise<AppointmentDto> => (await request<{ success: true; data: AppointmentDto }>(`/patient/organizations/${organizationId}/appointments`, { method: 'POST', headers: { Authorization: `Bearer ${accessToken}` }, body: JSON.stringify(input) })).data,
  listFamilyMembers: async (accessToken: string): Promise<PatientFamilyMemberDto[]> => (await request<{ success: true; data: PatientFamilyMemberDto[] }>('/patient/family-members', { method: 'GET', headers: { Authorization: `Bearer ${accessToken}` } })).data,
  createFamilyMember: async (accessToken: string, input: Partial<PatientFamilyMemberDto> & { firstName: string; lastName: string; relationship: string; dateOfBirth: string; documentId: string }): Promise<PatientFamilyMemberDto> => (await request<{ success: true; data: PatientFamilyMemberDto }>('/patient/family-members', { method: 'POST', headers: { Authorization: `Bearer ${accessToken}` }, body: JSON.stringify(input) })).data,
  patchFamilyMember: async (accessToken: string, familyMemberId: string, input: Partial<PatientFamilyMemberDto>): Promise<PatientFamilyMemberDto> => (await request<{ success: true; data: PatientFamilyMemberDto }>(`/patient/family-members/${familyMemberId}`, { method: 'PATCH', headers: { Authorization: `Bearer ${accessToken}` }, body: JSON.stringify(input) })).data,
  deleteFamilyMember: async (accessToken: string, familyMemberId: string): Promise<{ ok: true }> => (await request<{ success: true; data: { ok: true } }>(`/patient/family-members/${familyMemberId}`, { method: 'DELETE', headers: { Authorization: `Bearer ${accessToken}` } })).data,
  ...
  confirmAppointmentAttendance: async (accessToken: string, appointmentId: string): Promise<AppointmentDto> => (await request<{ success: true; data: AppointmentDto }>(`/patient/appointments/${appointmentId}/confirm-attendance`, { method: 'PATCH', headers: { Authorization: `Bearer ${accessToken}` }, body: JSON.stringify({ note: 'Confirmado por paciente' }) })).data,
  ...
  listWaitlist: async (accessToken: string): Promise<WaitlistRequestDto[]> => (await request<{ success: true; data: WaitlistRequestDto[] }>('/patient/waitlist', { method: 'GET', headers: { Authorization: `Bearer ${accessToken}` } })).data,
  createWaitlist: async (accessToken: string, input: { organizationId: string; specialtyId?: string; professionalId?: string; startDate: string; endDate: string; timeWindowStart?: string; timeWindowEnd?: string }): Promise<WaitlistRequestDto> => (await request<{ success: true; data: WaitlistRequestDto }>('/patient/waitlist', { method: 'POST', headers: { Authorization: `Bearer ${accessToken}` }, body: JSON.stringify(input) })).data,
  cancelWaitlist: async (accessToken: string, waitlistId: string): Promise<WaitlistRequestDto> => (await request<{ success: true; data: WaitlistRequestDto }>(`/patient/waitlist/${waitlistId}/cancel`, { method: 'PATCH', headers: { Authorization: `Bearer ${accessToken}` } })).data,
  listNotifications: async (accessToken: string, read?: 'read' | 'unread'): Promise<NotificationDto[]> => (await request<{ success: true; data: NotificationDto[] }>(`/notifications/me${read ? `?${new URLSearchParams({ read }).toString()}` : ''}`, { method: 'GET', headers: { Authorization: `Bearer ${accessToken}` } })).data,
  markNotificationRead: async (accessToken: string, notificationId: string): Promise<NotificationDto> => (await request<{ success: true; data: NotificationDto }>(`/notifications/${notificationId}/read`, { method: 'PATCH', headers: { Authorization: `Bearer ${accessToken}` } })).data
